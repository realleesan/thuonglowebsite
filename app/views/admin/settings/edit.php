...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        // header('Location: ?page=admin&module=settings&success=updated');
        // exit;
    }
}
...
<div class="settings-page settings-edit-page">
    <!-- Page Header -->
    <div class="page-header">
        <div class="page-header-left">
            ...
            <p class="page-description">Chỉnh sửa cài đặt: <strong><?= htmlspecialchars($setting['key']) ?></strong></p>
        </div>
    ...
    <div class="form-container">
        <form method="POST" class="admin-form">
            <div class="form-grid">
                ...
                <div class="form-column">
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            <label for="key">Tên cài đặt (Key)</label>
                            <input type="text" id="key" name="key" 
                                   value="<?= htmlspecialchars($setting['key']) ?>" 
                                   readonly class="readonly">
                            <small>Tên cài đặt không thể thay đổi</small>
                        </div>

                        <div class="form-group">
                            <label for="description" class="required">Mô tả</label>
                            <input type="text" id="description" name="description" 
                                   value="<?= htmlspecialchars($_POST['description'] ?? $setting['description']) ?>" 
                            ...
                            <select id="type" name="type" required onchange="updateValueField()">
                                <?php foreach ($setting_types as $type_key => $type_label): ?>
                                    <option value="<?= $type_key ?>" 
                                            <?= (($_POST['type'] ?? $setting['type']) == $type_key) ? 'selected' : '' ?>>
                                        <?= $type_label ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="form-section">
                        ...
                        <div class="info-item">
                            <label>Loại hiện tại:</label>
                            <div class="current-type">
                                <span class="type-badge type-<?= $setting['type'] ?>">
                                    <?= $setting_types[$setting['type']] ?? ucfirst($setting['type']) ?>
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-column">
                    <div class="form-section">
                        <h3 class="section-title">Giá Trị Mới</h3>
                        ...
                        <div class="form-group" id="value-group">
                            ...
                            <input type="text" id="value" name="value" 
                                   value="<?= htmlspecialchars($_POST['value'] ?? $setting['value']) ?>" 
                                   placeholder="Nhập giá trị..." required>
                            
                            <!-- Textarea (hidden by default) -->
                            <textarea id="value-textarea" name="value" rows="5" 
                                      placeholder="Nhập giá trị..." style="display: none;" required><?= htmlspecialchars($_POST['value'] ?? $setting['value']) ?></textarea>
                            
                            <!-- Boolean select (hidden by default) -->
                            <select id="value-boolean" name="value" style="display: none;" required>
                                <option value="1" <?= (($_POST['value'] ?? $setting['value']) == '1') ? 'selected' : '' ?>>Có (True)</option>
                                <option value="0" <?= (($_POST['value'] ?? $setting['value']) == '0') ? 'selected' : '' ?>>Không (False)</option>
                            </select>
                    ...
                    <div class="form-section">
                        <h3 class="section-title">Lưu Ý</h3>
                        <div class="alert alert-warning">
                            <i class="fas fa-exclamation-triangle"></i>
                            <ul>
                                <li>Thay đổi cài đặt có thể ảnh hưởng đến hoạt động của hệ thống</li>
                                <li>Hãy chắc chắn về giá trị trước khi lưu</li>
                                <li>Một số cài đặt có thể cần khởi động lại hệ thống</li>
                            </ul>
                        </div>
                    </div>
                </div>
...
function resetForm() {
    if (confirm('Bạn có chắc chắn muốn đặt lại form về giá trị ban đầu?')) {
        // Reset to original values
        document.getElementById('description').value = '<?= htmlspecialchars($setting['description']) ?>';
        document.getElementById('type').value = '<?= $setting['type'] ?>';
        document.getElementById('value').value = '<?= htmlspecialchars($setting['value']) ?>';
        document.getElementById('value-textarea').value = '<?= htmlspecialchars($setting['value']) ?>';
        document.getElementById('value-boolean').value = '<?= htmlspecialchars($setting['value']) ?>';
        updateValueField();
    }
}
