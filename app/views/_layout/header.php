    <!-- Top Banner -->
    <div class="top-banner">
        <div class="container">
            <p>Special welcome gift. Get 30% off your first purchase with code "Eduma". <a href="/courses/">Find out more!</a></p>
        </div>
    </div>

    <!-- Main Header -->
    <header class="main-header">
        <div class="container">
            <div class="header-content">
                <!-- Logo -->
                <div class="logo">
                    <a href="/">
                        <svg xmlns="http://www.w3.org/2000/svg" width="160" height="36" viewBox="0 0 160 36" fill="none">
                            <g clip-path="url(#clip0_127_426)">
                                <path d="M47.1932 0H0.276585C0.123831 0 0 0.123872 0 0.276677V9.50648C0 9.65928 0.123831 9.78316 0.276585 9.78316H47.1932C47.3459 9.78316 47.4698 9.65928 47.4698 9.50648V0.276677C47.4698 0.123872 47.3459 0 47.1932 0Z" fill="#356DF1"></path>
                                <path d="M33.6177 13.1079H0.276585C0.123831 13.1079 0 13.2318 0 13.3846V22.6144C0 22.7672 0.123831 22.8911 0.276585 22.8911H33.6177C33.7704 22.8911 33.8942 22.7672 33.8942 22.6144V13.3846C33.8942 13.2318 33.7704 13.1079 33.6177 13.1079Z" fill="#356DF1"></path>
                                <path d="M47.1932 26.2168H0.276585C0.123831 26.2168 0 26.3407 0 26.4935V35.7233C0 35.8761 0.123831 36 0.276585 36H47.1932C47.3459 36 47.4698 35.8761 47.4698 35.7233V26.4935C47.4698 26.3407 47.3459 26.2168 47.1932 26.2168Z" fill="#356DF1"></path>
                                <path d="M62.5754 25.221H73.648V28.3079H58.9307V8.67529H73.2605V11.7622H62.5754V25.221ZM62.2954 16.8361H72.0502V19.8373H62.2954V16.8361Z" fill="#212427"></path>
                                <path d="M83.5734 28.5021C82.1349 28.5021 80.8396 28.1843 79.6875 27.5486C78.5523 26.9274 77.6148 25.9991 76.9822 24.8699C76.33 23.7213 76.0039 22.3707 76.0039 20.8181C76.0039 19.2853 76.3312 17.9393 76.9857 16.78C77.6151 15.6449 78.5531 14.7114 79.6909 14.0875C80.883 13.4415 82.2212 13.1131 83.5769 13.134C84.7507 13.1151 85.9094 13.4002 86.9404 13.9617C87.931 14.5128 88.7158 15.3543 89.2948 16.4861C89.8739 17.618 90.1638 19.062 90.1646 20.8181C90.1646 22.5574 89.8842 23.9968 89.3234 25.1363C88.7626 26.2758 87.9915 27.1218 87.0102 27.6744C86.0273 28.2262 84.8817 28.5021 83.5734 28.5021ZM83.994 25.5296C84.7835 25.5436 85.5625 25.3463 86.2501 24.9579C86.9254 24.5642 87.4791 23.9917 87.8502 23.3036C88.2629 22.5425 88.4709 21.6872 88.4537 20.8215C88.4537 19.868 88.2525 19.0406 87.8502 18.3394C87.473 17.6632 86.9201 17.1015 86.2501 16.7136C85.5578 16.3363 84.7819 16.1386 83.9934 16.1386C83.205 16.1386 82.4291 16.3363 81.7368 16.7136C81.0672 17.1021 80.5146 17.6637 80.1367 18.3394C79.7344 19.0399 79.5336 19.8672 79.5344 20.8215C79.5168 21.6871 79.7243 22.5424 80.1367 23.3036C80.5085 23.9913 81.062 24.5635 81.7368 24.9579C82.4249 25.3462 83.2041 25.5435 83.994 25.5296ZM88.5657 28.3055V24.7716L88.6491 20.7895L88.3668 16.8062V7.49756H91.8996V28.3055H88.5657Z" fill="#212427"></path>
                                <path d="M102.831 28.5025C101.56 28.5025 100.439 28.264 99.4678 27.7868C98.4977 27.3123 97.699 26.548 97.182 25.5997C96.6303 24.6188 96.3545 23.3802 96.3545 21.884V13.3311H99.8907V21.4359C99.8907 22.7628 100.199 23.7583 100.815 24.4221C101.432 25.086 102.292 25.4179 103.395 25.4179C104.145 25.4353 104.888 25.2614 105.553 24.9126C106.173 24.5686 106.671 24.0394 106.977 23.3989C107.313 22.7259 107.481 21.8939 107.481 20.9031V13.3311H110.985V28.3082H107.653V24.2381L108.242 25.4957C107.745 26.4418 106.973 27.2146 106.027 27.7114C105.045 28.2397 103.946 28.5118 102.831 28.5025Z" fill="#212427"></path>
                                <path d="M134.481 13.1341C135.677 13.1341 136.738 13.3681 137.663 13.8361C138.595 14.3105 139.354 15.0655 139.835 15.9946C140.358 16.9672 140.62 18.2103 140.62 19.7241V28.3056H137.15V20.1734C137.15 18.8464 136.855 17.8555 136.266 17.2008C135.677 16.5461 134.839 16.2187 133.752 16.2187C133.042 16.205 132.34 16.3794 131.719 16.7241C131.12 17.0729 130.641 17.5961 130.347 18.2241C130.02 18.8887 129.857 19.7161 129.857 20.7061V28.3056H126.381V20.1734C126.381 18.8464 126.087 17.8555 125.498 17.2008C124.909 16.5461 124.071 16.2187 122.983 16.2187C122.273 16.2048 121.572 16.3792 120.951 16.7241C120.352 17.0729 119.874 17.5961 119.58 18.2241C119.252 18.8887 119.088 19.7161 119.088 20.7061V28.3056H115.613V13.3285H118.949V17.3392L118.332 16.133C118.826 15.1898 119.593 14.4176 120.533 13.9173C121.495 13.3952 122.602 13.1334 123.854 13.1318C125.256 13.1318 126.471 13.4779 127.499 14.1699C128.527 14.862 129.209 15.9291 129.545 17.3712L128.2 16.867C128.666 15.7641 129.46 14.8666 130.583 14.1745C131.705 13.4824 133.005 13.1356 134.481 13.1341Z" fill="#212427"></path>
                                <path d="M149.619 28.5025C148.498 28.5025 147.517 28.3119 146.676 27.9308C145.891 27.5908 145.217 27.036 144.733 26.3302C144.275 25.6465 144.036 24.8393 144.047 24.0162C144.036 23.2181 144.245 22.4322 144.65 21.7445C145.052 21.0722 145.711 20.5387 146.627 20.1439C147.543 19.7491 148.757 19.5524 150.27 19.5539H154.614V21.8817H150.522C149.345 21.8817 148.545 22.0722 148.122 22.4533C147.918 22.6336 147.756 22.8562 147.647 23.1056C147.538 23.355 147.485 23.6252 147.491 23.8973C147.484 24.1883 147.55 24.4763 147.682 24.7357C147.814 24.995 148.008 25.2176 148.248 25.3836C148.753 25.7578 149.454 25.9449 150.35 25.9449C151.16 25.9576 151.958 25.7543 152.663 25.3561C153.345 24.9633 153.874 24.3501 154.163 23.6172L154.723 25.7209C154.39 26.5961 153.753 27.3223 152.929 27.7674C152.068 28.2574 150.965 28.5025 149.619 28.5025ZM154.329 28.3058V25.2498L154.161 24.6324V19.3321C154.161 18.3039 153.848 17.5036 153.221 16.9312C152.595 16.3588 151.647 16.0738 150.376 16.076C149.547 16.0769 148.724 16.2092 147.937 16.4682C147.184 16.703 146.481 17.0738 145.862 17.5623L144.491 15.0105C145.36 14.3707 146.338 13.8952 147.378 13.6077C148.499 13.2878 149.66 13.1273 150.826 13.1309C153.012 13.1309 154.694 13.6545 155.872 14.7018C157.05 15.7491 157.638 17.3569 157.638 19.5254V28.3024L154.329 28.3058Z" fill="#212427"></path>
                            </g>
                            <defs>
                                <clipPath id="clip0_127_426">
                                    <rect width="160" height="36" fill="white"></rect>
                                </clipPath>
                            </defs>
                        </svg>
                    </a>
                </div>

                <!-- Categories Dropdown -->
                <div class="categories-dropdown">
                    <button class="categories-btn">
                        <span>Categories</span>
                        <svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1 1L5 5L9 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </button>
                    <div class="categories-menu">
                        <a href="/course-category/coaching/">Coaching</a>
                        <a href="/course-category/online-business/">Online Business</a>
                        <a href="/course-category/passive-income/">Passive Income</a>
                        <a href="/course-category/photography/">Photography</a>
                        <a href="/course-category/teacher-training/">Teacher Training</a>
                        <a href="/course-category/teaching-online/">Teaching Online</a>
                        <a href="/course-category/technology/">Technology</a>
                        <a href="/course-category/video-creation/">Video Creation</a>
                    </div>
                </div>

                <!-- Search Bar -->
                <div class="search-bar">
                    <form method="get" action="/courses/">
                        <button type="submit" class="search-btn">
                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12.5 11H11.71L11.43 10.73C12.41 9.59 13 8.11 13 6.5C13 2.91 10.09 0 6.5 0C2.91 0 0 2.91 0 6.5C0 10.09 2.91 13 6.5 13C8.11 13 9.59 12.41 10.73 11.43L11 11.71V12.5L16 17.49L17.49 16L12.5 11ZM6.5 11C4.01 11 2 8.99 2 6.5C2 4.01 4.01 2 6.5 2C8.99 2 11 4.01 11 6.5C11 8.99 8.99 11 6.5 11Z" fill="#6B7280"/>
                            </svg>
                        </button>
                        <input type="text" name="c_search" placeholder="Search for anythings..." class="search-input">
                        <input type="hidden" value="course" name="ref">
                    </form>
                </div>
            </div>
        </div>
    </header>

    <!-- Navigation -->
    <nav class="main-nav">
        <div class="container">
            <div class="nav-content">
                <!-- Main Menu -->
                <ul class="main-menu">
                    <li class="active"><a href="/">Home</a></li>
                    <li class="has-dropdown">
                        <a href="/courses/">Courses <svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1L5 5L9 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                        <div class="dropdown-menu">
                            <a href="/course-category/passive-income/">Courses Grid</a>
                            <a href="/course-category/photography/">Courses Grid 2 Columns</a>
                            <a href="/course-category/teaching-online/">Courses List</a>
                            <a href="/course-category/coaching/">Courses Load More</a>
                            <a href="/course-category/language-learning/">Courses Infinite Scroll</a>
                            <a href="/courses/offline-course/">Offline Course <span class="new-badge">New</span></a>
                        </div>
                    </li>
                    <li class="has-dropdown">
                        <a href="#">Pages <svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1L5 5L9 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                        <div class="dropdown-menu">
                            <a href="/landing-page-online/">Landing Page for Online Course</a>
                            <a href="/landing-page-offline/">Landing Page for Offline Course</a>
                            <a href="/instructors/">Instructors</a>
                            <a href="/portfolio-masonry/">Portfolio Masonry</a>
                            <a href="/gallery/">Gallery</a>
                            <a href="/faqs/">FAQs</a>
                            <a href="/shop/">Shop</a>
                            <a href="index.php?page=about">About Us</a>
                            <a href="/contact/">Contact</a>
                        </div>
                    </li>
                    <li><a href="/elements/">Elements</a></li>
                    <li class="has-dropdown">
                        <a href="/blog/">Blog <svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1L5 5L9 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                        <div class="dropdown-menu">
                            <a href="/category/blog/">Blog Grid</a>
                            <a href="/category/business/">Blog List</a>
                            <a href="/category/blog/">Blog Load More</a>
                            <a href="/category/design-branding/">Blog Infinite Scroll</a>
                        </div>
                    </li>
                    <li><a href="/events/">Events</a></li>
                    <li><a href="/backend-demo/">Backend Demo</a></li>
                </ul>

                <!-- Right Side Buttons -->
                <div class="header-buttons">
                    <a href="/get-started/" class="btn-get-started">Get Started</a>
                    <a href="index.php?page=login" class="btn-login">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M8 8C10.21 8 12 6.21 12 4C12 1.79 10.21 0 8 0C5.79 0 4 1.79 4 4C4 6.21 5.79 8 8 8ZM8 10C5.33 10 0 11.34 0 14V16H16V14C16 11.34 10.67 10 8 10Z" stroke="currentColor" stroke-width="1" fill="none"/>
                        </svg>
                        Log In
                    </a>
                </div>
            </div>
        </div>
    </nav>