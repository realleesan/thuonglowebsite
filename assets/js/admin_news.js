...
function executeBulkAction(action, itemIds) {
    ...
    if (confirm(confirmMessage)) {
        ...
        showNotification(`Đã ${actionText} ${itemIds.length} tin tức thành công`, 'success');
        
        // Reset selections
        document.querySelectorAll('.news-checkbox:checked').forEach(cb => cb.checked = false);
        document.getElementById('select-all').checked = false;
        updateBulkActionsState();
        
        // In real app, reload the page or update the table
        setTimeout(() => {
