...
<head>
    ...
    <style>
        ...
        .brand-status { padding: 4px 8px; border-radius: 12px; font-size: 12px; color: white; margin: 0 2px; }
...
echo "<div class='stats'>Total brands in database: " . count($allBrands) . "</div>";
...
=== BRANDS ANALYSIS ===
Total brands: " . count($allBrands) . "
Active + Visible (User Sidebar): " . count($activeVisible) . "
Active + Hidden: " . count($activeHidden) . "
Inactive + Visible: " . count($inactiveVisible) . "
Inactive + Hidden: " . count($inactiveHidden) . "

=== RECOMMENDATION ===
Admin filter config should show ALL brands for management
User sidebar correctly shows only Active + Visible brands
Use admin panel to toggle show_in_filter for brands
</div>";

echo "</div>";

echo "<div class='section'>
    <h2>🔧 Detailed Brand Data</h2>
    <div class='console'>";
...
foreach ($allBrands as $brand) {
    ...
    echo "  Status: " . $status . "\n";
    echo "  Show in Filter: " . ($showInFilter ? 'YES' : 'NO') . "\n";
    echo "  Product Count: " . $productCount . "\n";
    echo "  Sort Order: " . ($brand['sort_order'] ?? 0) . "\n";
    echo "  ---\n";
}
